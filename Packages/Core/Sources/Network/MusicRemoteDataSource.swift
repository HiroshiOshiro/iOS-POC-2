...
nonisolated public struct ITunesSearchResponse: Decodable, Sendable {
    public let results: [ITunesTrackDTO]
}

/// iTunes Search API の 1 要素（JSON）に対応する DTO。
/// モデル（`MusicTrack`）への変換は Data 層が行う。
nonisolated public struct ITunesTrackDTO: Decodable, Sendable {
...
nonisolated public protocol MusicRemoteDataSource: Sendable {
    nonisolated func search(term: String) async throws -> [ITunesTrackDTO]
}

/// 楽曲検索で起きうるエラー。
nonisolated public enum MusicRemoteError: Error {
    case invalidURL
    case httpStatus(Int)
}
...
nonisolated public struct ITunesMusicRemoteDataSource: MusicRemoteDataSource {
    public init() {}

    nonisolated public func search(term: String) async throws -> [ITunesTrackDTO] {
